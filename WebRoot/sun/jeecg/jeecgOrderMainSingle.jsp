...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'jeecgOrderMainSingleAction!datagrid.action',
			title : '一对多数据模型（明细在一个页面显示）',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			rownumbers : true,
		...
		cdescAdd = $('#cdescAdd').xheditor({
			...
			html5Upload : true,
			upMultiple : 4,
			upLinkUrl : 'jeecgOrderMainSingleAction!upload.action',
			upLinkExt : 'zip,rar,txt,doc,docx,xls,xlsx',
			upImgUrl : 'jeecgOrderMainSingleAction!upload.action',
	...
	function add() {
		$('#iframeDialog').dialog('setTitle', '新建订单主数据'); 
		iframeDialog.dialog('open');
		document.getElementById("iframe").src = "jeecgOrderMainSingleAction!toJeecgOrderMainSingleMainAdd.action";
	}
	function del() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'jeecgOrderMainSingleAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function edit() {
		var rows = datagrid.datagrid('getSelections');
		if (rows.length == 1) {
			$('#iframeDialog').dialog('setTitle', '编辑订单主数据'); 
			iframeDialog.dialog('open');
			document.getElementById("iframe").src = "jeecgOrderMainSingleAction!toJeecgOrderMainSingleMainEdit.action?obid="+rows[0].obid;
		} else {
	...
	function showCdesc(index) {
		...
		$.ajax({
			url : 'jeecgOrderMainSingleAction!showDesc.action',
			data : {
				obid : row.obid
			},
			dataType : 'json',
