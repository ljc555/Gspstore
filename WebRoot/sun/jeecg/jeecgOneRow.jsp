...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'jeecgOneRowAction!datagrid.action',
			title : '单表模型（行编辑样式）',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			...
			columns : [ [ 
			   ...
			   {field:'name',title:'用户名',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
			   ...
			   {field:'age',title:'年龄',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'numberbox',
							options : {required:true}
					},
			   ...
			   {field:'salary',title:'工资',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'numberbox',
							options : {required:true,precision:2,groupSeparator:','}
					},
					formatter:function(value,row,index){
						return row.salary;
					}
				},				
			   {field:'birthday',title:'生日',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'datebox',
							options : {required:true}
					},
			   ...
			   {field:'registerDt',title:'注册时间',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'datetimebox',
							options : {required:true}
					},
			   ...
			   {field:'content',title:'备注',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			...
			onAfterEdit : function(rowIndex, rowData, changes) {
				...
				if (inserted.length > 0) {
					url = 'jeecgOneRowAction!add.action';
				}
				if (updated.length > 0) {
					url = 'jeecgOneRowAction!edit.action';
				}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'jeecgOneRowAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
		...
		<form id="searchForm">
			<table>
				<tr>
					...
					<td>生日<input name="beginBirthday" style="width:100px;" class="easyui-datebox" editable="false"/>至<input name="endBirthday" style="width:100px;" class="easyui-datebox" editable="false"/>&nbsp;</td>
