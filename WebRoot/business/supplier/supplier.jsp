...
<script type="text/javascript" charset="utf-8">
    var searchForm;
	var datagrid;
	var supplierAddDialog;
	var supplierAddForm;
	var cdescAdd;
	var supplierEditDialog;
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'supplierAction!datagrid.action',
			title : '供应商信息列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			...
			{field:'ck',checkbox:true,
			   ...
			   {field:'suppliername',title:'供货商名称',align:'center',sortable:true,width:160,
					formatter:function(value,row,index){
						return row.suppliername;
					}
				},				
			   {field:'connectman',title:'联系人',align:'center',sortable:true,width:60,
					formatter:function(value,row,index){
						return row.connectman;
					}
				},				
			   {field:'phone',title:'联系电话',align:'center',sortable:true,width:90,
					formatter:function(value,row,index){
						return row.phone;
					}
				},				
			   {field:'address',title:'地址',align:'center',sortable:true,width:160,
					formatter:function(value,row,index){
						return row.address;
					}
				},				
			   {field:'fax',title:'传真',align:'center',sortable:true,width:90,
					formatter:function(value,row,index){
						return row.fax;
					}
				},				
			   {field:'email',title:'邮箱',align:'center',sortable:true,width:140,
					formatter:function(value,row,index){
						return row.email;
					}
				},				
			   ...
			   {field:'meno',title:'备注',align:'center',sortable:true,width:150,
					formatter:function(value,row,index){
						return row.meno;
					}
				}				
		...
		supplierAddForm = $('#supplierAddForm').form({
			url : 'supplierAction!add.action',
			success : function(data) {
				...
				if (json && json.success) {
					...
					supplierAddDialog.dialog('close');
				} else {
					$.messager.show({
						title : '失败',
						msg : '操作失败！'
		...
		supplierAddDialog = $('#supplierAddDialog').show().dialog({
			title : '添加供应商信息',
			modal : true,
			closed : true,
			maximizable : true,
		...
		cdescAdd = $('#cdescAdd').xheditor({
			...
			html5Upload : true,
			upMultiple : 4,
			upLinkUrl : 'supplierAction!upload.action',
			upLinkExt : 'zip,rar,txt,doc,docx,xls,xlsx',
			upImgUrl : 'supplierAction!upload.action',
			upImgExt : 'jpg,jpeg,gif,png'
		});

		supplierEditForm = $('#supplierEditForm').form({
			url : 'supplierAction!edit.action',
			success : function(data) {
				var json = $.parseJSON(data);
				if (json && json.success) {
					$.messager.show({
					...
					datagrid.datagrid('reload');
					supplierEditDialog.dialog('close');
				} else {
					$.messager.show({
						title : '失败',
						msg : '操作失败！'
		...
		supplierEditDialog = $('#supplierEditDialog').show().dialog({
			title : '编辑供应商信息',
			modal : true,
			closed : true,
			maximizable : true,
	...
	function add() {
		supplierAddForm.find('input,textarea').val('');
		$('div.validatebox-tip').remove();
		supplierAddDialog.dialog('open');
	}
	function del() {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'supplierAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
	...
	function edit() {
		...
		if (rows.length == 1) {
			...
			$.ajax({
				url : 'supplierAction!showDesc.action',
				data : {
				...
				cache : false,
				success : function(response) {
					supplierEditForm.find('input,textarea').val('');
					supplierEditForm.form('load', response);
					$('div.validatebox-tip').remove();
					supplierEditDialog.dialog('open');
		...
		$.ajax({
			...
			success : function(response) {
				if (response && response.cdesc) {
					...
					$.messager.alert('提示', '没有供应商信息描述！', 'error');
				}
				$.messager.progress('close');
			}
		});
...
<body class="easyui-layout">
	<div region="north" border="false" title="过滤条件" collapsed="true"  style="height: 60px;overflow: hidden;display: none;" align="left">
		<form id="searchForm">
			<table class="tableForm datagrid-toolbar" style="width: 100%;height: 100%;">
				<tr>
					<td>供货商名称</td>
					<td><input name="suppliername" style="width:120px;" class="easyui-validatebox"/></td>
					<td>联系人</td>
	...
	<div id="supplierAddDialog" style="display: none;width: 350px;height: 310px;padding:10px;" align="center">
		<form id="supplierAddForm" method="post">
			<table class="tableForm">
							<tr>
		 						<td class="tdLeft">供货商名称</td>
		 						<td class="tdRight">
									<input name="suppliername" type="text" maxlength="80" class="easyui-validatebox" data-options="required:true" missingMessage="请填写供货商名称"  style="width: 155px;"/>
								</td>
							</tr>
							<tr>
							...
							<tr>
		 						...
		 						<td class="tdRight">
									<input name="fax" type="text" maxlength="15" class="easyui-validatebox" data-options="required:true" missingMessage="请填写传真"  style="width: 155px;"/>
								</td>
							</tr>
							<tr>
	...
	<div id="supplierEditDialog" style="display: none;width: 350px;height: 310px;padding:10px;" align="center">
