...
<script type="text/javascript" charset="utf-8">
    ...
    function   _search() {
      if  ($('#searchForm').form('validate')){
	       ...
	       document.getElementById("iframe").src = "articleAction!viewArticleTrackData.action?"+querystring;
	    }
	}
		...
		<form id="searchForm" method="post" target="iframe">
			<table class="tableForm datagrid-toolbar" style="width: 100%;height: 100%;">
				<tr>
					<td width="30px">品种</td>
					<td>
					<input type="hidden" name="reportQueryForm.articleid" id="articlePagearticleid" />
					<select id="articlePage.articlename" name="articlePage.articlename" type="text"  class="easyui-combobox" data-options="required:true,
										...
										onSelect: function(rec){ 
										   $('#articlePagearticleid').val(rec.articleid);
										}
					...
					<td width="30px">批号</td>
					<td>
					<input type="text" name="reportQueryForm.articlespec" style="width:100px;" />
		 		    </td>
					<td width="60px">时间区段</td>
					<td>
					   <input id="begindate" name="reportQueryForm.begindate" style="width:117px;" class="easyui-datetimebox" data-options="required:true"/>
					  - <input id="enddate" name="reportQueryForm.enddate" style="width:117px;" class="easyui-datetimebox" data-options="required:true" />
					</td>
