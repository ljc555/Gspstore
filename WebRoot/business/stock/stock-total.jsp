...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'stockAction!datagridTotal.action',
			title : '库存汇总列表',
			pagination : true,
			pagePosition : 'bottom',
			rownumbers : true,
			...
			columns : [ [ 
			   ...
			   {field:'amount',title:'成本金额',align:'center',sortable:false,width:80,
					formatter:function(value,row,index){
						return parseFloat(row.amount,10).toFixed(2);
					}
				},				
			   {field:'costprice',title:'平均成本价',align:'center',sortable:false,width:80,
					formatter:function(value,row,index){
					    if (row.num>0)
						  return (parseFloat(row.amount,10)/parseInt(row.num,10)).toFixed(2);
						else
						  return 0;
					}
				}				
