...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : 'catelogAction!treegrid.action',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
					...
					if (node) {
						treegrid.treegrid('expandAll', node.catelogcode);
					} else {
				...
				handler : function() {
					var node = treegrid.treegrid('getSelected');
					if (node) {
						treegrid.treegrid('collapseAll', node.catelogcode);
					} else {
				...
				handler : function() {
					if (editRow != undefined) {
						treegrid.treegrid('endEdit', editRow.catelogcode);
					}
				}
			}, '-', {
				text : '取消编辑',
				...
				handler : function() {
					if (editRow) {
						treegrid.treegrid('cancelEdit', editRow.catelogcode);
						var p = treegrid.treegrid('getParent', editRow.catelogcode);
						if (p) {
							treegrid.treegrid('reload', p.catelogcode);
						} else {
							treegrid.treegrid('reload');
						}
						editRow = undefined;
			...
			},
			{
				...
				editor : {
					type : 'combotree',
					options : {
						url : 'catelogAction!catelogTreeRecursive.action',
						animate : false,
			...
			onDblClickRow : function(row) {
				...
				if (editRow == undefined) {
					treegrid.treegrid('beginEdit', row.catelogcode);
					editRow = row;
					editType = 'edit';
					treegrid.treegrid('unselectAll');
				}
			...
			onAfterEdit : function(row, changes) {
				...
				var url = '';
				if (editType == 'add') {
					url = 'catelogAction!add.action';
				}
				if (editType == 'edit') {
					url = 'catelogAction!edit.action';
				}
				...
				$.ajax({
					...
					success : function(r) {
						if (r.success) {
							...
							treegrid.treegrid('beginEdit', editRow.catelogcode);
			...
			onContextMenu : function(e, row) {
				...
				$(this).treegrid('select', row.catelogcode);
				$('#menu').menu('show', {
					left : e.pageX,
					top : e.pageY
				});
	...
	function edit() {
		var node = treegrid.treegrid('getSelected');
		if (node && node.catelogcode) {
			if (editRow != undefined) {
				treegrid.treegrid('endEdit', editRow.catelogcode);
			}

			if (editRow == undefined) {
				treegrid.treegrid('beginEdit', node.catelogcode);
				editRow = node;
				editType = 'edit';
			}
		} else {
	...
	function append() {
		...
		if (editRow == undefined) {
			...
			var row = [ {
				...
				curl : '',
				cseq : 10,
				cpid : node == null ? '' : node.catelogcode
			} ];
			treegrid.treegrid('append', {
				parent : node == null ? '' : node.catelogcode,
				data : row
			});

			editRow = row[0];
			editType = 'add';
			treegrid.treegrid('select', editRow.catelogcode);
			treegrid.treegrid('beginEdit', editRow.catelogcode);
		}
	}
	function remove() {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.cname + '】？', function(b) {
				if (b) {
					$.ajax({
						url : 'catelogAction!delete.action',
						data : {
							catelogcode : node.catelogcode
						},
						cache : false,
						dataType : "json",
						success : function(r) {
