...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : 'jeecgGroupAction!treegrid.action',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
					...
					if (node) {
						treegrid.treegrid('expandAll', node.obid);
					} else {
				...
				handler : function() {
					var node = treegrid.treegrid('getSelected');
					if (node) {
						treegrid.treegrid('collapseAll', node.obid);
					} else {
				...
				handler : function() {
					if (editRow != undefined) {
						treegrid.treegrid('endEdit', editRow.obid);
					}
				}
			}, '-', {
				text : '取消编辑',
				...
				handler : function() {
					if (editRow) {
						treegrid.treegrid('cancelEdit', editRow.obid);
						var p = treegrid.treegrid('getParent', editRow.obid);
						if (p) {
							treegrid.treegrid('reload', p.obid);
						} else {
							treegrid.treegrid('reload');
						}
						editRow = undefined;
			...
			onDblClickRow : function(row) {
				...
				if (editRow == undefined) {
					treegrid.treegrid('beginEdit', row.obid);
					editRow = row;
					editType = 'edit';
					treegrid.treegrid('unselectAll');
				}
			...
			onAfterEdit : function(row, changes) {
				...
				var url = '';
				if (editType == 'add') {
					url = 'jeecgGroupAction!add.action';
				}
				if (editType == 'edit') {
					url = 'jeecgGroupAction!edit.action';
				}
				...
				$.ajax({
					...
					success : function(r) {
						if (r.success) {
							...
							treegrid.treegrid('beginEdit', editRow.obid);
	...
	function edit() {
		...
		if (node && node.obid) {
			...
			if (editRow == undefined) {
				treegrid.treegrid('beginEdit', node.obid);
				editRow = node;
				editType = 'edit';
			}
		} else {
	...
	function append() {
		...
		if (editRow == undefined) {
			...
			var row = [ {
				...
				cname : '组织机构',
				curl : '',
				cpid : node == null ? '' : node.obid
			} ];
			treegrid.treegrid('append', {
				parent : node == null ? '' : node.obid,
				data : row
			});

			editRow = row[0];
			editType = 'add';
			treegrid.treegrid('select', editRow.obid);
			treegrid.treegrid('beginEdit', editRow.obid);
		}
	}
	function remove() {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.cname + '】？', function(b) {
				if (b) {
					$.ajax({
						url : 'jeecgGroupAction!delete.action',
						data : {
							obid : node.obid
						},
						cache : false,
