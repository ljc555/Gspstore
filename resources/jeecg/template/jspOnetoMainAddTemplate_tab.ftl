...
$(document).ready(function() {
	...
	<#list subTab as sub>
	 ...
	 //提交Form
	 myForm = $('#myForm').form({
			url : '${entityName?uncap_first}Action!addMain.action',
...
<body>
        <div id="chk_idAll">
            <form name="myForm" id="myForm" method="post">
             <div>
              <table>
                  <tr bgcolor="#CCE6D1">
                    <td colspan="8"  align="center" style="font-size:14px;"><strong>${ftl_description}</strong></td>
                  </tr>
					<#list mainColums as po>
					<#if po_index%4==0>
					<tr>
 					</#if>
                   	 	<td align="left"><#if po.filedComment?length lt 7 ><#if po.filedComment?length lt 7 >${po.filedComment}<#else>${po.filedComment[0..6]}</#if><#else>${po.filedComment[0..6]}</#if></td>
                    	<td align="left">
                    		<input name="${po.fieldName}" type="text" class="${po.classType}" maxlength="${po.charmaxLength}" data-options="${po.optionType}"/>
                    	</td>
					<#if (po_index+1)%4==0>
					</tr>
					<#else>
		...
		<div class="tabs">
				<#list subTab as sub>
				 ...
				 <div title="${sub.ftlDescription}">
				  <table>
                  <tr bgcolor="#CCE6D1">
                    <td align="left" ><strong>&nbsp;&nbsp;${sub.ftlDescription}&nbsp;&nbsp;</strong>
                        ...
                        <tbody id="add_${sub.entityName?uncap_first}_table">
	                        <tr>
							  ...
							  <#list sub.subColums as po>
								...
								<#if check==0>
								<td align="left"><input name="${sub.entityName?uncap_first}List[0].${po.fieldName}" maxlength="${po.charmaxLength}" type="text" class="easyui-validatebox" style="width:120px;" ></td>
								</#if>
                              </#list>
	                        </tr>
                       </tbody>
